...
export default function MobileFeatureCarousel() {
  ...
  useEffect(() => {
    ...
    const animateDots = (index: number) => {
      ...
      dotsRef.current.querySelectorAll<HTMLElement>('.m-rail-dot').forEach((d, i) => {
        gsap.to(d, {
          width: i === index ? 22 : 6,
          backgroundColor: i === index ? '#FF5E1A' : 'rgba(255,244,221,0.10)',
    ...
    const peekCards = () => {
      if (!rail) return;
      const cards = Array.from(rail.querySelectorAll<HTMLElement>('.m-fcard'));
      const railWidth = rail.clientWidth;
      cards.forEach((card) => {
        const cardOffsetLeft = card.offsetLeft - rail.scrollLeft;
        const cardCenter = cardOffsetLeft + card.offsetWidth / 2;
        const dist = Math.abs(cardCenter - railWidth / 2);
        const progress = Math.min(1, dist / (railWidth * 0.75));
        gsap.to(card, {
    ...
    const handleScroll = () => {
      ...
      if (maxScroll > 0) {
        animateDots(Math.round((rail.scrollLeft / maxScroll) * 2));
      }
      peekCards();
    };

    // Initialise peek state after cards animate in
    gsap.delayedCall(1.2, peekCards);
